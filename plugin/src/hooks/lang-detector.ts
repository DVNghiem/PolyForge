...
const LANG_PATTERNS: Array<{ pattern: RegExp; language: string }> = [
  ...
  { pattern: /go\.mod|go\.sum|\.go\b|package\s+main|func\s+\w+/i, language: 'go' },
  { pattern: /pom\.xml|build\.gradle|\.java\b|public\s+class|import\s+(?:java|jakarta|org\.spring)/i, language: 'java' },
  { pattern: /cmake|makefile|\.cpp\b|\.hpp\b|#include\s*[<"]|int\s+main\s*\(/i, language: 'cpp' },
  { pattern: /gemfile|\.rb\b|require\s+['"]rails|class\s+\w+\s*<\s*(?:Application|ActiveRecord)/i, language: 'ruby' },
