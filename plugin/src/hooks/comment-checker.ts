import { PfPluginApi, CommentViolation } from '../types.js';
...
function appendViolationSummary(content: string, violations: CommentViolation[]): string {
  ...
  return `${content}\n\n---\n⚠️ [PF Comment Checker] Found ${violations.length} AI slop comment(s):\n${details}\n\nConsider removing these obvious/narrating comments to keep code clean.`;
}

export function registerCommentChecker(api: PfPluginApi): void {
  api.registerHook(
    'tool_result_persist',
    (payload: ToolResultPayload): ToolResultPayload | undefined => {
      const config = getConfig(api);
      if (!config.comment_checker_enabled) return undefined;

      const { content } = payload;
      if (typeof content !== 'string' || content.trim().length === 0) return undefined;

      const fileHint = extractFileHint(payload);
      if (hasNonCodeExtension(fileHint) || contentLooksNonCode(content)) return undefined;
    ...
    {
      name: 'polyforge.comment-checker',
